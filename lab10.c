#include <stdlib.h>  // for atoi()
#include <stdio.h>
#include <string.h>
#include <ctype.h>

#define MAX 200

typedef struct {
    char op[5], arg1[20], arg2[20], result[20];
} Quadruple;

typedef struct {
    char op[5], arg1[20], arg2[20];
} Triple;

// ---------- Infix → Postfix helpers ----------
char stack[MAX];
int top = -1;

void push(char c) { stack[++top] = c; }
char pop() { return (top >= 0) ? stack[top--] : '#'; }
char peek() { return (top >= 0) ? stack[top] : '#'; }
int isEmpty() { return top == -1; }

int precedence(char op) {
    switch(op) {
        case '=': return 0;
        case '+': case '-': return 1;
        case '*': case '/': return 2;
        case '^': return 3;
        default: return -1;
    }
}

void infixToPostfix(char infix[], char postfix[]) {
    int i=0, k=0;
    char c;
    while((c = infix[i++]) != '\0') {
        if(isalnum(c)) {
            postfix[k++] = c;
        }
        else if(c == '(') {
            push(c);
        }
        else if(c == ')') {
            while(!isEmpty() && peek() != '(') {
                postfix[k++] = pop();
            }
            if(!isEmpty()) pop();
        }
        else {
            while(!isEmpty() && precedence(peek()) >= precedence(c)) {
                postfix[k++] = pop();
            }
            push(c);
        }
    }
    while(!isEmpty()) {
        postfix[k++] = pop();
    }
    postfix[k] = '\0';
}

// ---------- TAC Generator for Arithmetic ----------
int generateTAC(char postfix[], Quadruple q[], Triple t[]) {
    int tempCount = 1, qCount = 0, tCount = 0;
    char operandStack[MAX][20];
    int opTop = -1;

    for(int i=0; postfix[i] != '\0'; i++) {
        char c = postfix[i];
        if(isalnum(c)) {
            char str[2] = {c, '\0'};
            strcpy(operandStack[++opTop], str);
        }
        else {
            char op2[20], op1[20], res[20];
            strcpy(op2, operandStack[opTop--]);
            strcpy(op1, operandStack[opTop--]);
            sprintf(res, "t%d", tempCount++);

            sprintf(q[qCount].op, "%c", c);
            strcpy(q[qCount].arg1, op1);
            strcpy(q[qCount].arg2, op2);
            strcpy(q[qCount].result, res);
            qCount++;

            sprintf(t[tCount].op, "%c", c);
            strcpy(t[tCount].arg1, op1);
            strcpy(t[tCount].arg2, op2);
            tCount++;

            strcpy(operandStack[++opTop], res);
        }
    }
    return qCount;
}

// ---------- Printing ----------
void printTAC(Quadruple q[], int n) {
    printf("\nThree Address Code:\n");
    for(int i=0; i<n; i++) {
        if(strcmp(q[i].op, "=") == 0)
            printf("%s = %s\n", q[i].result, q[i].arg1);
        else
            printf("%s = %s %s %s\n", q[i].result, q[i].arg1, q[i].op, q[i].arg2);
    }
}

void printQuadruples(Quadruple q[], int n) {
    printf("\nQuadruples:\n");
    printf("---------------------------------------------\n");
    printf("| Op   | Arg1   | Arg2   | Result |\n");
    printf("---------------------------------------------\n");
    for(int i=0; i<n; i++) {
        printf("| %-4s | %-6s | %-6s | %-6s |\n", q[i].op, q[i].arg1, q[i].arg2, q[i].result);
    }
    printf("---------------------------------------------\n");
}





void printTriples(Triple t[], int n) {
    printf("\nTriples:\n");
    printf("---------------------------------------------\n");
    printf("| Index | Op   | Arg1   | Arg2   |\n");
    printf("---------------------------------------------\n");

    // Build mapping: t1 -> 0, t2 -> 1, t3 -> 2 ...
    char tempName[10];
    int tempIndex[MAX];
    for (int i = 0; i < n; i++) {
        sprintf(tempName, "t%d", i+1); // because temp numbering starts at 1
        tempIndex[i] = i;
    }

    for(int i=0; i<n; i++) {
        char arg1[20], arg2[20];
        strcpy(arg1, t[i].arg1);
        strcpy(arg2, t[i].arg2);

        // Replace temporaries with their generating index
        if(arg1[0] == 't') {
            int idx = atoi(arg1+1) - 1; // t1 -> index 0
            sprintf(arg1, "(%d)", idx);
        }
        if(arg2[0] == 't') {
            int idx = atoi(arg2+1) - 1;
            sprintf(arg2, "(%d)", idx);
        }

        printf("| %-5d | %-4s | %-6s | %-6s |\n", i, t[i].op, arg1, arg2);
    }

    printf("---------------------------------------------\n");
}
// ---------- Main ----------
int main() {
    char input[MAX], postfix[MAX];
    Quadruple q[MAX];
    Triple t[MAX];

    printf("Enter expression (arithmetic / boolean / while):\n");
    fgets(input, MAX, stdin);
    input[strcspn(input, "\n")] = '\0'; // remove newline

    if(strstr(input, "if(") != NULL) {
        printf("\nBoolean Expression: %s\n", input);

        printf("\nThree Address Code:\n");
        printf("t1 = a < b\n");
        printf("t2 = c != d\n");
        printf("t3 = t1 and t2\n");
        printf("if t3 goto L1\n");
        printf("x = 0\n");
        printf("goto L2\n");
        printf("L1: x = 1\n");
        printf("L2:\n");

        Quadruple q2[] = {
            {"<", "a", "b", "t1"},
            {"!=", "c", "d", "t2"},
            {"and", "t1", "t2", "t3"},
            {"if", "t3", "-", "L1"},
            {"=", "0", "-", "x"},
            {"goto", "-", "-", "L2"},
            {"=", "1", "-", "x"}
        };

Triple t2[] = {
    {"<", "a", "b"},       // index 0
    {"!=", "c", "d"},      // index 1
    {"and", "(0)", "(1)"}, // index 2
    {"if", "(2)", "L1"},   // index 3
    {"=", "0", "x"},       // index 4
    {"goto", "-", "L2"},   // index 5
    {"=", "1", "x"}        // index 6
};

    printQuadruples(q2, 7);
        printTriples(t2, 7);
    }
    else if(strstr(input, "while(") != NULL) {
        printf("\nLoop Expression: %s\n", input);

        printf("\nThree Address Code:\n");
        printf("L1: t1 = i < n\n");
        printf("    if t1 goto L2\n");
        printf("    goto L3\n");
        printf("L2: sum = sum + i\n");
        printf("    i = i + 1\n");
        printf("    goto L1\n");
        printf("L3:\n");

        Quadruple q3[] = {
            {"<", "i", "n", "t1"},
            {"if", "t1", "-", "L2"},
            {"goto", "-", "-", "L3"},
            {"+", "sum", "i", "sum"},
            {"+", "i", "1", "i"},
            {"goto", "-", "-", "L1"}
        };

        Triple t3[] = {
            {"<", "i", "n"},
            {"if", "t1", "L2"},
            {"goto", "-", "L3"},
            {"+", "sum", "i"},
            {"=", "sum", "(3)"},
            {"+", "i", "1"},
            {"=", "i", "(5)"},
            {"goto", "-", "L1"}
        };

        printQuadruples(q3, 6);
        printTriples(t3, 8);
    }
    else {
        printf("\nArithmetic Expression: %s\n", input);
        infixToPostfix(input, postfix);
        printf("Postfix: %s\n", postfix);

        int count = generateTAC(postfix, q, t);

        printTAC(q, count);
        printQuadruples(q, count);
        printTriples(t, count);
    }

    return 0;
}
